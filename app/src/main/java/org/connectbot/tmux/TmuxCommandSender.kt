...
class TmuxCommandSender(
    ...
    suspend fun listWindows(format: String = "#{window_id} #{window_name}"): String {
        return sendCommand("list-windows -F '$format'").output
    }
    ...
    suspend fun listPanes(
        ...
        return sendCommand("list-panes -t '$windowId' -F '$format'").output
    }
    ...
    suspend fun killWindow(windowId: String): TmuxCommandResponse {
        return sendCommand("kill-window -t '$windowId'")
    }

    suspend fun selectWindow(windowId: String): TmuxCommandResponse {
        return sendCommand("select-window -t '$windowId'")
    }
     ...
     */
    suspend fun sendKeysHex(paneId: String, hexCodes: String): TmuxCommandResponse {
        return sendCommand("send-keys -t '$paneId' -H $hexCodes")
    }

    suspend fun resizeClient(width: Int, height: Int): TmuxCommandResponse {
        return sendCommand("refresh-client -C ${width}x${height}")
    }

    suspend fun enableFlowControl(pauseAfterSeconds: Int = 1): TmuxCommandResponse {
        return sendCommand("refresh-client -f pause-after=$pauseAfterSeconds")
    }

    suspend fun resumePane(paneId: String): TmuxCommandResponse {
        return sendCommand("refresh-client -A '$paneId:continue'")
    }

    suspend fun getVersion(): String {
        return sendCommand("display-message -p '#{version}'").output.trim()
    }

    suspend fun capturePane(paneId: String): String {
        return sendCommand("capture-pane -p -e -t '$paneId'").output
    }
